...
3. Сервер регистрирует соединение в Redis: `session:{user_id}` → `node_id:connection_id`.
...
| `id` | string (uuid) | Уникальный идентификатор сообщения. Используется для ack и дедупликации. |
...
На основе `ack` сервер публикует `receipt.events` (delivered) в Kafka.
...
| 4002 | Duplicate Connection | Обнаружено дублирующее соединение с того же устройства. Старое закрыто. |
...
```
session:{user_id}        Hash  { node_id, connection_id, connected_at }  TTL: 5 min (обновляется heartbeat)
conn:{connection_id}     Hash  { user_id, node_id, connected_at }        TTL: 5 min
presence:{user_id}       String  "online" | "offline"                    TTL: 65 sec (обновляется ping)
```

### Алгоритм доставки события конкретному пользователю

1. Gateway получает событие из Kafka (например, `message.created`).
2. Для каждого участника чата читает `session:{user_id}` из Redis.
3. Если запись есть и `node_id` совпадает с текущим узлом — отправляет напрямую через WS.
4. Если `node_id` другой — публикует сообщение в Redis Pub/Sub канал `node:{node_id}`,
...
Явной подписки на чаты не требуется — сервер определяет релевантные чаты через Chat Service
или кэш при установке соединения.
...
|---|---|
| Максимум WS-соединений на пользователя | 5 (разные устройства) |
...
Gateway → читает session:{user_B_id} из Redis
...
Gateway → публикует receipt.events (delivered) в Kafka
...
Message Service → обновляет статусы → публикует receipt.events (read) в Kafka
